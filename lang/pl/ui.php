<?php

return[
     // Navbar
     'home' => 'Home',
     'newAnnouncement' => 'stwórz ogłoszenie',
     'auditorArea' => 'strefa kontroli',
     'allAnnouncement' => 'wszystkie ogłoszenia',
     'welcome' => 'Witaj',
     'login' => 'Zaloguj się',
     'registered' => 'Zarejestruj się',
     'logout' => 'Wyloguj się',
     'search' => 'szukaj ogłoszeń',
 
     // Inserisci un annuncio
     'enterAnnouncement' => 'stwórz ogłoszenie',
 
     // Tutti gli Annunci
         // 'allAnnouncement' => 'Tutti gli Annunci',
     'noFound' => 'nie znaleźliśmy ogłoszeń',
 
     // Scopri di più
     'fastShipping' => 'Szypka spedycja',
     'evidence' => 'Pròba Plus gratis na 30 dni',
     'expedition' => 'Spedycja standardowa',
     'gratis' => 'Gratis',
     'reviews' => 'Oceny (0)',
     'packaging' => 'Opakowanie nie uciążliwe dla środowiska',
     'expeditions' => 'Wszyskie opakowania użyte podczas produkcji są 100% poddane recyklingowi.',
     'rendered' => 'łatwe zwroty',
     'goods' => 'Masz 14 dni roboczych od daty otrzymania towaru na skorzystanie z prawa zwrotu.',
     'securePayments' => 'Bezpieczne płatności',
     'payments' => 'Płatności kartą kredytową, Paypal, bezpośrednim przelewem bankowym, Scalapay i za pobraniem.',
     'buyNow' => 'Kup teraz',
     'addToCart' => 'Dodaj do wózka',
    // 'allAnnouncement' => 'Tutti gli Annunci',
     'backToHome' => 'Wróć do strony głównej',
 
     // Sezione con anteprima annuncio
     'preview' => 'zapowiedź',
     'requiredFields' => 'Pola oznaczone gwiazdą* są obowiązkowe',
     'nameArticle' => 'Nazwa ogłoszenia *',
     'category' => 'kategoria *',
     'chooseCategory' => 'Wybierz kategorię ogłoszenia *',
     'selectImage' => 'Wybierz zdjęcia *',
     'clear' => 'Usuń',
     'describe' => 'Opisz swoj ogłoszenie *',
     'price' => 'Cena *',
     'post' => 'Opublikuj ogłoszenie',
 
     // Email revisore
     'requestToCollaborate' => 'Poprosił o współpracę z nam.',
     'name' => 'imię',
     'email' => 'Email',
     'makeAuditor' => 'Stwórz kontrolera',
 
     // Pagina revisore
     'rejectAd' => 'odrzucić ogłoszenie',
     'undoChange' => 'cofnij ostatnią zmianę',
     'acceptAd' => 'zaakceptuj ogłoszenie',
 
     // Home
     'slogan' => 'Zakupy u ciebie w domu',
     'allCategories' => 'Wszysykie Kategorje',
     'clothing' => 'Ubrania',
     'furniture' => 'Meble',
     'electronics' => 'Elektronika',
     'film' => 'Filmy',
     'fitness' => 'Fitness',
     'gardening' => 'Ogrodnictwo',
     'games' => 'Zabawy',
     'books' => 'Książki',
     'engines' => 'motoryzacja',
     'music' => 'Muzyka',
     'latestArticles' => 'Najnowsze ogłoszenia',
 
     // Scopri di più categorie
     'noAnnouncement' => 'Nie ma ogłoszeń.',
 
     // Login
     'emailAddress' => 'Adres Email',
     'password' => 'hasło',
     'remember' => 'Zapamiętaj',
     'noAccount' => 'Nie masz konta? Zarejestruj się',
     // 'login' => 'Accedi',
 
     // Registrati
     'username' => 'Twoje imię',
     // 'emailAddress' => 'Indirizzo Email',
     // 'password' => 'Password',
     'confirmPassword' => 'Potwierdź hasło',
     // 'registered' => 'Registrati',
 
     // Footer
     'contacts' => 'Kontakty',
     'warning' => 'Uwaga',
     'message' => 'klikając na przycisk "kontynuuj" twoje dane osobowe będą wysyłane przez email do admina strony Presto.it aby zweryfikować twòj wniosek.',
     'continues' => 'kontynuuj',
     'cancel' => 'Anuluj',
     'becomeRevisor' => 'Zostań kontrolerem',
     'changeLanguage' => 'Zmien język',
     'social' => 'śledź nas na naszych mediach społecznościowych:'
];
